<script>
	...
	import { STUDY_ENABLED } from '~/study/env';
</script>
...
<div id="description">
	...
	{/if}
	<!--
		The YouTube embed is dropped from study builds. Loading it makes the
		participant's browser contact youtube.com, googleads.g.doubleclick.net and
		static.doubleclick.net — i.e. Google ad infrastructure — which we cannot
		put in front of research participants, and which would falsify the IRB
		data-flow answer. Everything else in the article is untouched, so the
		control arm still gets TE's full written explainer.
	-->
