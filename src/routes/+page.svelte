<script lang="ts">
	...
	import { MOCK_MODEL } from '~/study/env';

	// Self-hosted: the ONNX WebAssembly runtime is served from our own build
	// rather than jsDelivr. Upstream points this at a CDN, which would leave a
	// live third-party dependency in the middle of a study session — the exact
	// thing self-hosting is meant to remove.
	ort.env.wasm.wasmPaths = `${base}/ort/`;
	ort.env.logLevel = 'error';

	// Same reasoning for the tokenizer: upstream pulls `Xenova/gpt2` from the
	// Hugging Face CDN at runtime. Vendored into static/models/ and pinned to
	// local-only so a participant's session never depends on huggingface.co
	// being reachable or fast.
	...
	onMount(async () => {
		...
		if (MOCK_MODEL) {
			// E2E mode: skip the 657 MB ONNX download entirely and stay on TE's
			// own `fakeRunWithCachedData` path, which `runModelOrCache` below
			// already selects whenever `modelSession` is unset. Clearing
			// `isFetchingModel` is what keeps the input box enabled — leaving it
			// true would disable the form and there would be nothing to test.
